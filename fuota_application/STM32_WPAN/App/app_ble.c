...
FUOTA_APP_ConnHandleNotEvt_t FUOTAHandleNotification;

static const char a_GapDeviceName[] = {  'U', 'p', 'd', 'a', 't', 'e', ' ', 'M', 'e' }; /* Gap Device Name */

/* Advertising Data */
uint8_t a_AdvData[27] =
{
  10, AD_TYPE_COMPLETE_LOCAL_NAME, 'U', 'p', 'd', 'a', 't', 'e', ' ', 'M', 'e',  /* Complete name */
...
/* USER CODE BEGIN GV */
uint32_t ConnectionIntervalMin = CONN_INT_MS(1000);
uint32_t ConnectionIntervalMax = CONN_INT_MS(1000);
...
/* USER CODE BEGIN PFP */
static void FillAdvertisingData(uint8_t* adv_data, uint8_t length);
...
SVCCTL_UserEvtFlowStatus_t SVCCTL_App_Notification(void *p_Pckt)
{
  ...
  switch (p_event_pckt->evt)
  {
    case HCI_DISCONNECTION_COMPLETE_EVT_CODE:
    {
      ...
      FUOTAHandleNotification.EvtOpcode = FUOTA_DISCON_HANDLE_EVT;
      FUOTAHandleNotification.ConnectionHandle = p_disconnection_complete_event->Connection_Handle;
      FUOTA_APP_EvtRx(&FUOTAHandleNotification);
    ...
    case HCI_LE_META_EVT_CODE:
    {
      ...
      switch (p_meta_evt->subevent)
      {
        ...
        case HCI_LE_ENHANCED_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          FUOTAHandleNotification.EvtOpcode = FUOTA_CONN_HANDLE_EVT;
          FUOTAHandleNotification.ConnectionHandle = p_enhanced_conn_complete->Connection_Handle;
          FUOTA_APP_EvtRx(&FUOTAHandleNotification);
        ...
        case HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          FUOTAHandleNotification.EvtOpcode = FUOTA_CONN_HANDLE_EVT;
          FUOTAHandleNotification.ConnectionHandle = p_conn_complete->Connection_Handle;
          FUOTA_APP_EvtRx(&FUOTAHandleNotification);
...
void APP_BLE_Procedure_Gap_Peripheral(ProcGapPeripheralId_t ProcGapPeripheralId)
{
  ...
  switch(ProcGapPeripheralId)
  {
    ...
    case PROC_GAP_PERIPH_CONN_PARAM_UPDATE:
    {
      ...
      paramA = ConnectionIntervalMin;
      paramB = ConnectionIntervalMax;
...
/* USER CODE BEGIN FD*/
/**
  * @brief Update the BLE connection interval
  * @param min: Minimum connection interval in milliseconds
  * @param min: Maximum connection interval in milliseconds
  */
void APP_BLE_UpdateConnectionInterval(uint32_t min, uint32_t max)
{
  if (ConnectionIntervalMin != min || ConnectionIntervalMax != max)
  {
    ConnectionIntervalMin = min;
    ConnectionIntervalMax = max;
...
static void Ble_Hci_Gap_Gatt_Init(void)
{
  ...
  FillAdvertisingData(&a_AdvData[0], sizeof(a_AdvData));
...
/**
  * @brief Fills the advertising data buffer with the specified data
  * @param adv_data: Pointer to the array where advertising data will be filled
...
static void FillAdvertisingData(uint8_t* adv_data, uint8_t length)
{
  ...
  while(i < length)
  {
    ...
    case AD_TYPE_MANUFACTURER_SPECIFIC_DATA:
      {
        memcpy(&adv_data[i+4], "UPDATE ME :)", 12);
        break;
      }
    default:
      break;
    }
    // i += 1 (memorize the length) + ad_data (type + length)
